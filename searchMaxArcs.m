...
% 返回newSchedule=[1工件号 2工件工序号 3机器号 4开工时间 5完工时间 6关键工序标志位 7最大弧数]
% 中间schedule=[7标记位 8索引位 9最大弧数]
function newSchedule = searchMaxArcs(schedule)
    schedule(:,7)=-1; %是否已经调度的标志位 -1为没调度 0为待调度，1为已调度 初始化为-1
    schedule(:,8)=1:size(schedule,1); %索引位
    schedule(:,9)=0; %最大弧数位 初始化为0
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    while(size(schedule(schedule(:,7)==0,7),1)~=0) %如果待调度序列还有时：
        ...
        if ~isempty(thisPointJPScheduleRow) %如果找到了JP
            JPArcsNum=schedule(thisPointJPScheduleRow,9);
        ...
        if thisProcessInSMSRow==1 %如果是第一行，则MP不存在
            ...
            MPArcsNum=schedule(sameMachineSchedule(thisProcessInSMSRow-1,8),9);
        end

        %更新最大弧数、标记位
        schedule(nowSNSchedule(point,8),9)=max(JPArcsNum,MPArcsNum)+1;
